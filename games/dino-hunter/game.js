...
document.addEventListener('DOMContentLoaded', () => {
  if (typeof GameNavigation !== 'undefined') {
    window.gameNav = new GameNavigation('dino-hunter', {
      screens: ['game'],
      initialScreen: 'game',
      gameName: 'Dino Hunter'
    });
  }
});
...
const DINOS = [
  ...
  },
  {
    ...
    facts: [
      ...
      'Back plates may have changed colour.',
      'Spiked tail was called a "thagomizer".'
    ]
  },
  {
    id: 'pteranodon',
    ...
    color: '#1E88E5',
    facts: [
      'Wingspan wider than a small airplane!',
  ...
  },
  {
    ...
    color: '#795548',
    facts: [
      'Its armour was tougher than bone!',
...
function update(dt) {
  ...
  G.dinos.forEach(d => {
    ...
    if (d.x - 48 < 0 || d.x + 48 > W) {
      d.vx *= -1;
    }
    if (d.y - 30 < 48 || d.y + 30 > H - 38) {
      d.vy *= -1;
    }
...
function spawnDino() {
  ...
  el.style.cssText =
    'position:absolute;transform:translate(-50%,-50%);pointer-events:none;text-align:center;transition:opacity .2s,transform .2s;z-index:5';
  el.innerHTML =
...
function draw() {
  ...
  [
    ...
    ctx.beginPath();
    ctx.moveTo((l * W) / 900, H * 0.56);
    ctx.lineTo((p * W) / 900, H * 0.2);
    ctx.lineTo((r * W) / 900, H * 0.56);
    ctx.fill();
  });
  ...
  [
    ...
    ctx.beginPath();
    ctx.moveTo((l * W) / 900, H * 0.3);
    ctx.lineTo((p * W) / 900, H * 0.2);
    ctx.lineTo((r * W) / 900, H * 0.3);
    ctx.fill();
  });

  // Grass
  ctx.fillStyle = '#5D8A3C';
  ctx.fillRect(0, H * 0.59, W, 16);
  ctx.fillStyle = '#4A7030';
  for (let x = 0; x < W; x += 20) {
    ctx.beginPath();
    ctx.moveTo(x, H * 0.59);
    ctx.lineTo(x + 10, H * 0.59 - 12);
    ctx.lineTo(x + 20, H * 0.59);
    ctx.fill();
  }
  // Ground
  ...
  roundRect(ctx, 8, H - 40, 110, 30, 6, true, false);
...
function showFactPanel(dino) {
  ...
  el.id = 'dh-fact';
  el.style.cssText =
    'position:absolute;inset:0;z-index:50;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.65)';
  el.innerHTML = `

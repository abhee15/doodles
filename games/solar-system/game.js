...
function updateScene() {
    ...
    PLANETS.forEach(p => {
        ...
        if (planetObjects[p.id]) {
            ...
            obj.gfx.x = px;
            obj.gfx.y = py;
            if (obj.label) obj.label.setPosition(px, py + p.radius + 9);
            if (obj.hitArea) obj.hitArea.setPosition(px, py);
            ...
            if (obj.label) obj.label.setDepth(depth + 1);
            if (obj.hitArea) obj.hitArea.setDepth(depth + 2);
            if (planetGlows[p.id]) planetGlows[p.id].setDepth(depth - 1);
        }
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
        const p = particles[i];
        if (!p || !p.active) { particles.splice(i, 1); continue; }
        p.x += p.vx;
        p.y += p.vy;
        p.alpha -= p.fade;
        if (p.alpha <= 0) { p.destroy(); particles.splice(i, 1); }
    }
}
...
function drawPlanets(scene) {
    PLANETS.forEach(p => {
        ...
        hitArea.on('pointerout', () => {
            ...
            if (!isPlanetTarget(p.id)) showGlowRing(scene, p.id, false);
        });
...
function showFactCard(scene, obj, targetX, targetY) {
    ...
    closeBtn.on('pointerup', () => { factCardGroup && factCardGroup.destroy(true); factCardGroup = null; });
...
function showQuestion(scene, q) {
    if (questionPanel) { questionPanel.destroy(true); questionPanel = null; }
...
function handlePlanetClick(scene, planet) {
    ...
    if (currentQuestion.type !== 'click') return;
...
function resolveAnswer(scene, correct, planet, choice) {
    if (correct) {
        ...
        if (glow) {
            ...
            if (p) glow.strokeCircle(0, 0, p.radius + 5);
        }
...
function clearFeedback(scene) {
    if (feedbackGroup) { feedbackGroup.destroy(true); feedbackGroup = null; }
}
...
function clearQuizUI(scene) {
    ...
    progressDots.forEach(d => d.destroy());
    progressDots = [];
    if (celebrationGroup) { celebrationGroup.destroy(true); celebrationGroup = null; }
}
...
function addCelebBtn(scene, x, y, label, cb) {
    ...
    btn.add(hit);
    hit.on('pointerover', () => { btn.setScale(1.06); bg.clear(); bg.fillStyle(0x2A5ABA, 1); bg.fillRoundedRect(-72, -18, 144, 36, 10); bg.lineStyle(2, 0x80B8FF, 1); bg.strokeRoundedRect(-72, -18, 144, 36, 10); });
    hit.on('pointerout', () => { btn.setScale(1); bg.clear(); bg.fillStyle(0x1A3A8A, 1); bg.fillRoundedRect(-72, -18, 144, 36, 10); bg.lineStyle(2, 0x5A90FF, 1); bg.strokeRoundedRect(-72, -18, 144, 36, 10); });
    hit.on('pointerup', cb);

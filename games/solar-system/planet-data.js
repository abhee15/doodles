...
const QUESTIONS = [
  ...
  },
  {
    ...
    answer: 'uranus',
    successMsg:
      'Uranus is an ice giant — its interior is filled with icy water, methane, and ammonia!'
  },
  {
    id: 'q_type_5',
    level: 2,
  ...
  },
  {
    ...
    answer: 'mars',
    successMsg:
      'Mars is red because of iron rust! It also has the tallest volcano in the solar system.'
  },
  {
    id: 'q_feat_7',
    level: 4,
  ...
  },
  {
    ...
    successMsg:
      "Europa's underground ocean may be the best place to search for alien life in the solar system!"
  },
  ...
  },
  {
    ...
    answer: 'neptune',
    successMsg:
      "Neptune takes 165 years per orbit — it's completed less than one orbit since it was discovered!"
  }
];
...
function getDynamicQuestions(level) {
  ...
  if (level === 2) {
    ...
    if (nonRocky.length > 0) {
      ...
      qs.push({
        ...
        type: 'choice',
        choices: _sh([target.id, ...rocky3.map(p => p.id)]),
        answer: target.id,
  ...
  if (level === 3) {
    ...
    [
      ...
      qs.push({
        id: `dyn_bigger_${pA.id}_${pB.id}`,
        level: 3,
        category: 'Size',
        text: `Which planet is bigger: ${pA.name} or ${pB.name}?`,
        type: 'choice',
        choices: _sh([pA.name, pB.name]),
        answer: bigger.name,
    ...
    PLANETS.filter(p => p.sizeRank <= 5).forEach(p =>
      qs.push({
        id: `dyn_sizelabel_${p.id}`,
        level: 3,
        category: 'Size',
  ...
  if (level === 4) {
    [
      {
        pid: 'mercury',
        text: 'I orbit the Sun in only 88 Earth days — the fastest of all. Which planet am I?',
        msg: 'Mercury is the fastest-orbiting planet — up to 59 km per second!'
      },
      {
        pid: 'venus',
        text: 'I spin backwards and my day is longer than my year. Which planet am I?',
        msg: 'Venus rotates so slowly that a day on it outlasts its own year!'
      },
      {
        pid: 'earth',
        text: 'I have liquid oceans, plate tectonics, and a magnetic shield. Which planet?',
        msg: "Earth's unique combination of features makes it perfect for life!"
      },
      {
        pid: 'mars',
        text: 'My sunsets are blue, and I host the tallest volcano in the solar system. Which planet?',
        msg: "Iron-rich dust makes Mars's sky pink and its sunsets beautifully blue!"
      },
      {
        pid: 'jupiter',
        text: "My magnetic field is 20,000× stronger than Earth's. Which giant planet?",
        msg: "Jupiter's colossal magnetic field is the strongest of any planet!"
      },
      {
        pid: 'saturn',
        text: "I'm less dense than water — I would float in a big enough ocean! Which planet?",
        msg: "Saturn is so light it would float — it's mostly gas!"
      },
      {
        pid: 'uranus',
        text: 'One of my poles gets 42 continuous years of sunlight, then 42 years of darkness. Which planet?',
        msg: 'Uranus is tilted 98° — it rolls around the Sun like a bowling ball!'
      },
      {
        pid: 'neptune',
        text: 'I was predicted by mathematics before anyone ever saw me. Which planet?',
  ...
  if (level === 5) {
    ...
    PLANETS.forEach(p => {
      ...
      qs.push({
        id: `dyn_mooncount_${p.id}`,
        level: 5,
        category: 'Moons',
        text: `How many known moons does ${p.name} have?`,
        type: 'choice',
        choices,
        answer: String(p.moons),
    ...
    [
      ...
      qs.push({
        id: `dyn_moonname_${mq.pid}`,
        level: 5,
        category: 'Moons',
        text: `What is ${planet.name}'s most famous moon called?`,
        type: 'choice',
        choices: _sh([mq.moon, ...mq.others]),
        answer: mq.moon,

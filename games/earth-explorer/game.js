...
function showMainMenu(scene) {
  ...
  scene.add
    .text(450, 160, 'Learn Continents & Oceans!', {
      fontSize: '24px',
      fill: '#4A4A4A',
      fontFamily: 'Arial'
    })
...
function startGame(scene, difficulty) {
  ...
  // Score
  scoreText = scene.add
    .text(800, 30, `Score: ${score}/${totalQuestions}`, {
      fontSize: '22px',
      fill: '#FFFFFF',
      fontFamily: 'Arial',
      fontStyle: 'bold',
...
function nextQuestion(scene, pool) {
  ...
  questionText = scene.add
    .text(450, 30, `Click on: ${data.name}`, {
      fontSize: '32px',
      fill: '#FFFFFF',
      fontFamily: 'Arial',
      fontStyle: 'bold',
...
function handleClick(scene, id) {
  ...
  if (id === currentQuestion) {
    ...
    if (mapObjects[id].label) {
      ...
      const label = scene.add
        .text(bounds.centerX, bounds.centerY, mapObjects[id].data.name, {
          fontSize: '16px',
          fill: '#000000',
          fontFamily: 'Arial',
          fontStyle: 'bold',
...
function showFunFact(scene, fact, onClose) {
  const overlay = scene.add
    .rectangle(450, 325, 900, 650, 0x000000, 0.7)
    .setDepth(2000)
    .setInteractive();
  const modalBg = scene.add
    .rectangle(450, 325, 600, 300, 0xffffff)
    .setDepth(2001)
    .setStrokeStyle(4, 0xa44a3f);
  ...
  const btn = scene.add
    .rectangle(450, 410, 200, 50, 0x10b981)
    .setDepth(2002)
    .setInteractive({ useHandCursor: true });
  const btnText = scene.add
    .text(450, 410, 'Continue', {
      fontSize: '22px',
      fill: '#FFFFFF',
      fontFamily: 'Arial',
      fontStyle: 'bold'
...
function showResults(scene) {
  ...
  scene.add
    .text(450, 180, `You found ${score} out of ${totalQuestions}!`, {
      fontSize: '32px',
      fill: '#4A4A4A',
      fontFamily: 'Arial'
    })
  ...
  scene.add
    .text(450, 240, '⭐'.repeat(stars) + '☆'.repeat(3 - stars), {
      fontSize: '56px'
    })
    .setOrigin(0.5);
